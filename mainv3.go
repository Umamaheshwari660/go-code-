...
// the whole point of higher order function is that  either or both
// 1- it returns another function

func foo2() func(p int) string {
	c := func(param int) string {
		return "hello world"
	}
	return c
}

func bar2(c func(p int) bool) {
	//c()
}

// defer is always added before the function
// and it makes that function execute just before the return statment
func sub() {
	defer func() {
		fmt.Println("2 in the end")
	}()
	...
	return // function execution stops as soon as you encounter the return statement
}
